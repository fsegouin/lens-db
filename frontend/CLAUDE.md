...
│   ├── pool.ts                 # createPool: tiny pg Pool (max 4) with the root CA pinned in db-ca.ts; also used by mcp-server
│   ├── db-ca.ts                # pinned root CA for the DB TLS chain (generated by infra/db/gen-certs.sh)
...
Schema location: `src/db/schema.ts`. Drizzle config: `drizzle.config.ts` (output: `./drizzle`). It loads `.env`/`.env.local`, then splits `DATABASE_URL` into host/port/user/password/database and pins the root CA bundle (`src/db/db-ca.ts`) because drizzle-kit cannot verify a private CA chain from a bare URL.
...
- `cameras.front_view` names the straight-on cut-out the compare page's size view (`CameraSizeComparison`) draws to scale, with its outline crop in pixels. `scripts/camera-front-views.mjs` fills it: a photo qualifies when it has a transparent ground and its outline has the width-to-height ratio of the `Dimensions` spec (parsed by `lib/camera-dimensions.ts`) within 8%, which rejects angled shots. Without a front view on both cameras the size view renders nothing
...
```bash
DATABASE_URL=          # pgbouncer transaction pooler URL, port 6543 (required; see infra/db/README.md)
