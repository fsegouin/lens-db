...
function normalizeBrand(name) {
  return name
    ...
    .replace(/^Asahi (Super-Multi-Coated |Super-)?Takumar/i, 'Pentax')
    .replace(/^Asahi (Pentax|Opt\.)/i, 'Pentax')
    .replace(/^Nippon Kogaku/i, 'Nikon')
    .replace(/^Nikkorex/i, 'Nikon')
    .replace(/^Nikkor/i, 'Nikon')
    .replace(/^NIKKOR/i, 'Nikon')
    .replace(/^Fujinon/i, 'Fuji')
    .replace(/^Fujifilm/i, 'Fuji')
    .replace(/^Fujica/i, 'Fuji')
    .replace(/^Mamiya[/-]SEKOR/i, 'Mamiya')
    .replace(/^Mamiya[/-]Sekor/i, 'Mamiya')
    .replace(/^(ROLLEINAR|Rolleinar|Zoom-Rolleinar|Reflex-Rolleinar|HFT-Rolleinar|F-Rolleinar|APO-Rolleinar|Rollei-HFT)/i, 'Rollei')
    .replace(/^Komuranon/i, 'Komura')
    .replace(/^Komura-FX/i, 'Komura')
    .replace(/^Exaktar/i, 'Exakta')
    .replace(/^Tamron-F/i, 'Tamron')
