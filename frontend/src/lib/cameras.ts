...
// Filed under the camera's own tag as well as the broad ones, so an edit to
// fields only this page shows can refresh it without emptying every list.
const _getCameraBySlug = (slug: string) =>
  unstable_cache(
    async () => {
    ...
    ["camera-by-slug", slug],
    { revalidate: 2592000, tags: ["cameras", "lenses", entityTag("camera", slug)] },
  )();
