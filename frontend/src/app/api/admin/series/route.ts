...
export async function GET(request: NextRequest) {
  ...
  const conditions = q ? buildNameSearch(lensSeries.name, q) : [];
  const where = conditions.length > 0 ? and(...conditions) : undefined;

  const direction = orderParam === "desc" ? desc : asc;
  const lensCountExpr = sql<number>`count(${lensSeriesMemberships.lensId})`;
  const orderByMap: Record<string, ReturnType<typeof asc>> = {
    name: direction(lensSeries.name),
    lensCount: direction(lensCountExpr),
  };
  const orderBy = (sortParam && orderByMap[sortParam]) || asc(lensSeries.name);
