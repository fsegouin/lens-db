...
Imports the parsed data into the PostgreSQL database. Requires `DATABASE_URL` environment variable.

The Python scripts hand `DATABASE_URL` straight to psycopg2. The database presents a certificate signed by a private root CA, so either use `sslmode=require` in that URL or add `sslrootcert=<path to infra/db/certs/ca.crt>` alongside `sslmode=verify-full`.
...
- `ebay-resolve-action.mjs`: the resolve pass of the Browse API price pipeline (`.github/workflows/ebay-browse-prices.yml`, which also drives the asking pass over `/api/cron/ebay-asking` with plain `curl`). Reads each due listing's own `/itm/<id>` page in Playwright, several readers at once, and POSTs the verdicts to `/api/cron/ebay-resolve`, which decides: the Browse API reports a listing its seller pulled in the same shape as a sale, and only the page says which. Env: `RESOLVE_BUDGET` (pages, default 800), `RESOLVE_BATCH` (default 50), `RESOLVE_CONCURRENCY` (readers, default 4). When Chrome itself dies the pass relaunches it with half as many readers, down to one, and stops after three deaths; listings left unread are handed out again next time. Exits non-zero when more than half the pages were blocked.
...
- `kenrockwell-scrape.mjs` — crawler for Ken Rockwell's lens reviews, one brand directory at a time (Nikon, Canon, Sony, Fuji, LEICA, Zeiss, the third parties). Reads through the Wayback Machine because the live site sits behind a Cloudflare challenge; takes the newest capture that returned 200, since from mid-2025 the archive's own crawler gets the wall too. Extracts typed facts (years, elements/groups, blades, close focus, magnification, filter, size, weight, dated prices) and keeps his Introduction, Identification, Performance and Recommendations text **locally, for writing our own descriptions from**, never for publishing. Output `kenrockwell.json` and the `kenrockwell-cache/` directory are gitignored. Args: `--dirs`, `--limit`, `--delay`, `--out`, `--reparse` (rebuild from cache), `--debug`. Map the result onto our rows with `frontend/scripts/map-kenrockwell-to-lenses.mjs`, which reports matched reviews, reviews of lenses we do not have, and which empty columns each review could fill.
